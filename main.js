...
/* ---------- 凭据本地存储 ---------- */
const STORAGE_KEY = 'xmovAvatarDemoCredentials'
function loadCredentials() {
  try {
    const saved = JSON.parse(localStorage.getItem(STORAGE_KEY) || 'null')
    if (saved?.appId?.trim() && saved?.appSecret?.trim()) return saved
  } catch (_) {}
  return { appId: SDK_CONFIG.appId, appSecret: SDK_CONFIG.appSecret }
}
function saveCredentials() {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ appId: els.appIdInput.value.trim(), appSecret: els.appSecretInput.value.trim() })) } catch (_) {}
}
...
function init() {
  ...
  els.appIdInput.value = creds.appId; els.appSecretInput.value = creds.appSecret

  // 按钮事件
  els.btnInit.addEventListener('click', () => {
    if (!els.appIdInput.value.trim() || !els.appSecretInput.value.trim()) {
      els.appIdInput.classList.toggle('is-invalid', !els.appIdInput.value.trim())
      els.appSecretInput.classList.toggle('is-invalid', !els.appSecretInput.value.trim())
    ...
    initSDK({ appId: els.appIdInput.value.trim(), appSecret: els.appSecretInput.value.trim(), gatewayServer: SDK_CONFIG.gatewayServer })
  })
  ...
  // 凭据输入
  els.appIdInput.addEventListener('input', () => { els.appIdInput.classList.remove('is-invalid'); saveCredentials(); updateButtons() })
  els.appSecretInput.addEventListener('input', () => { els.appSecretInput.classList.remove('is-invalid'); saveCredentials(); updateButtons() })
